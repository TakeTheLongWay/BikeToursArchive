...
set "STRAVA_URL=https://www.strava.com/dashboard"
...
REM Chrome öffnen
if exist "%CHROME_EXE%" (
    REM Öffnet beide URLs in Tabs eines neuen Fensters
    start "" "%CHROME_EXE%" --new-window "%URL%" "%STRAVA_URL%" 
) else (
    echo [WARN] Chrome nicht gefunden. 
    echo Oeffne URLs im Standardbrowser... [cite: 3]
    start "" "%URL%" 
    start "" "%STRAVA_URL%"
)
